...
def delete_item(db: Session, item_id: int):
    item_cursor = db.query(Item).filter(Item.id == item_id)

    if not item_cursor.first():
        raise ValueError(f"There is no item with ID {item_id}")
    else:
        item_cursor.delete()
        db.commit()
...
def update_item(db: Session, item_id: int, item_update: schemas.ItemUpdate):
    ...
    if not item:
        raise ValueError(f"There isn't any item with ID {item_id}")
