...
from schema.schema import NewEntry, ResEntry, Role
...
@router.post(
    "/user/entries/", response_model=ResEntry, status_code=status.HTTP_201_CREATED
)
async def save_entries(entry: NewEntry, token: str = Depends(oauth2_scheme)):
    try:
        ...
        if entry.number_of_calories is not None and int(entry.number_of_calories) < int(
            settings.EXPECTED_CALORIES_PER_DAY
        ):
        ...
        raise HTTPException(
            ...
            detail="Entry was not saved to the database",
        )


# Endpoint for retrieving entries
@router.get(
    "/user/entries/", response_model=List[ResEntry], status_code=status.HTTP_200_OK
)
async def get_entries(
    ...
    try:
        ...
        if role == Role.USER:
            all_entries = (
                db.query(Entry)
                .filter(Entry.role == Role.USER.value)
                .offset(offset)
                .limit(per_page)
                .all()
            )
            ...
            raise HTTPException(
                ...
                detail="User with the role specified was not found",
            )
...
@router.put(
    "/user/entries/{entry_id}/", response_model=ResEntry, status_code=status.HTTP_200_OK
)
async def update_entries(
    entry_id: int, entry: ResEntry, token: str = Depends(oauth2_scheme)
):
    try:
        user_from_token = await get_user_from_token(token)
        role = Role(user_from_token["role"])
        ...
        if entry_to_update is None:
            raise HTTPException(
                status_code=400, detail=f"Entry with the id {entry_id} was not found"
            )
    ...
    try:
        ...
        if entry_to_delete is None:
            raise HTTPException(
                ...
                detail=f"Entry with the given id {entry_id} is not found",
            )
